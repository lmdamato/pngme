...
impl Png {
    pub const STANDARD_HEADER: &'static [u8; 8] = &[0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a];
...
pub trait IPng {
    fn header(&self) -> &[u8; 8] { Png::STANDARD_HEADER }
...
impl IPng for Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let chunk_type_bytes = ChunkType::from_str(chunk_type).map(|ct| ct.bytes())?;

        for el in &self.chunks {
            if el.chunk_type().bytes() == chunk_type_bytes {
                if res.is_none() {
                    res = Some(el.clone());
                } else {
                    Err(anyhow!("Found multiple chunks with the same chunk type"))?;
                }
            } else {
                new_chunks.push(el.clone());
            }
        }
        ...
        match res {
            ...
            _ => Err(anyhow!("Couldn't find chunk with the specified chunk type")),
        }
    }
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        let chunk_type_bytes = ChunkType::from_str(chunk_type)
            .map(|ct| ct.bytes())
            .ok()?;

        self.chunks.iter().find(|el| el.chunk_type().bytes() == chunk_type_bytes)
    }

    fn as_bytes(&self) -> Vec<u8> {
        let content_bytes: Vec<u8> = self.chunks
            .iter()
            .map(|c| c.as_bytes())
            .flatten()
            .collect();

        [Png::STANDARD_HEADER.to_vec(), content_bytes].concat()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        fn is_valid_header(header: &[u8; 8]) -> bool {
            header == Png::STANDARD_HEADER
        }

        fn next_chunk_size(value: &[u8], start: usize) -> usize {
            let size_x: [u8; 4] = value[start..start+4].try_into().unwrap();
            u32::from_be_bytes(size_x).try_into().unwrap()
        }

        if value.len() < 8 {
            Err(anyhow!("Malformed input: need at least the size of the header"))?;
        }

        if !is_valid_header(&value[..8].try_into().unwrap()) {
        ...
        while idx < value.len() {
            let next_chunk_size = next_chunk_size(value, idx);
            let until = idx + next_chunk_size + 12;

            if until > value.len() {
                Err(anyhow!("Malformed input: chunk length beyond array size"))?;
            }

            let next_chunk = Chunk::try_from(&value[idx..until]);
            match next_chunk {
                Ok(c) => res.push(c),
                Err(e) => return Err(e),
            }
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", String::from_utf8_lossy(self.as_bytes().as_slice()).to_string())
    }
}
